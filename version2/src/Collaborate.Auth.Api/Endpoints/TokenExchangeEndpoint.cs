...
public static class TokenExchangeEndpoint
{
        ...
        CancellationToken ct)
    {
        ...
        // The caller authenticated as itself via JwtBearer (see Program.cs) — its own token's
        // `sub` is the internal service identity we check delegation grants against. We never
        // trust a service-identity string supplied in the request body; it can only be the
        // thing the caller's own validated token says it is.
